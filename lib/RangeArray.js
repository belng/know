...
function diff (a, b) {
	...
	function pushR(value) {
		if (value === -Infinity) value = START;
		if (value === +Infinity) value = END;
	...
	function step() {
		...
		na = na === START ? -Infinity : (na === END ? +Infinity : na);
		nb = nb === START ? -Infinity : (nb === END ? +Infinity : nb);
...
class RangeArray {
	constructor(ranges) {
		...
		Object.defineProperty(this, "length", { get: () => ranges.length });
	}
	...
	*/
	...
	subtract(ranges) {
		this.arr = diff(new Stepper(this), new Stepper(ranges));
	}

	add(range) {
		let startPos = this.indexOf(range[0]), endPos;
		if (range.length === 3) {
			if (
				!this.arr[startPos] ||
	...
	put(range) {
		let startPos = this.indexOf(range[0]);
		this.arr.splice(startPos, 0, range);
	}
