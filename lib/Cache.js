	...
	*/
	...
	deleteEntity(id) {
		...
		for (let key in keyValues) {
			if (keyValues[key].item === this.entities[id]) {
	...
	*/
	put(changes) {
		...
		if (changes.knowledge) { for (let key in changes.knowledge) {
			...
			if (!(items instanceof OrderedArray)) {
				let slice = keyToSlice(key),
					order = (slice.join || slice.link) ?
						[ slice.type, slice.order ] :
						[ slice.order ];

				items = new OrderedArray(order, items);
			}
	...
	getEntity(id) {
		...
		let value = this.get(...([ "entities" ].concat(arguments)));
		if (typeof value === "undefined") {
			this.put({ queries: { entities: { [id]: true } } });
			return { type: "loading", id: id };
		} else if (value === false) {
