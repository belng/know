	...
	*/
	putEntity(entity, excludeKeys) {
		...
		if (this.queries.entities[entity.id]) {
			...
			this.queries.entities[entity.id].forEach(watchObject => {
	...
	*/
	put(changes) {
		...
		function mergeChanges(ch) {
			if (ch.entities) {
				allChanges.entities = {};
				jsonop(allChanges.entities, ch.entities);
			}
			if (ch.knowledge) {
				allChanges.knowledge = {};
				jsonop(allChanges.knowledge, ch.knowledge);
			}
			if (ch.queries) {
				allChanges.queries = {};
				jsonop(allChanges.queries, ch.queries);
			}
		}
		...
		if (changes.app) {
			...
			this._processAppWatches(changes.app);
		}
		...
		if (allChanges.knowledge) {
			for (const key in allChanges.knowledge) {
				if (this._queryCallbacks[key]) {
					let removeIndex = [], i = 0;
					this._queryCallbacks[key].forEach(({ range, callback, watch }) => {  // eslint-disable-line no-loop-func
		...
		if (allChanges.entities) { for (const id in allChanges.entities) {
			if (this._queryCallbacks.entities[id]) {
				...
				this._queryCallbacks[id].forEach(({ callback, watch }) => { // eslint-disable-line no-loop-func
	...
	query (key, range, callback) {
		let dRange = new RangeArray([ range ]), r = dRange.get(0),
			{ items, gaps } = this.getItemsGaps(key, r);
	...
	watch (key, range, callback) {
		...
		let { items } = this.getItemsGaps(key, range, callback, true),
		...
		return () => this.removeQueryCallback(key, watchObj);
	}
	...
	getEntity(id, callback) {
		const result = this.findEntity(id);
		if (result) {
	...
	watchEntity(id, callback) {
		const result = this.findEntity(id), watch = { callback, watch: true};
		if (!callback) { throw Error('ERR_WATCH_NO_CALLBACK'); }
		if (result) callback(null, result);
		this._queryCallbacks.entities = this._queryCallbacks.entities || {};
		(this._queryCallbacks.entities[id] = this._queryCallbacks.entities[id] || []).push(watch);
		this._queryCallbacks.entities[id].push(watch);

		return () => this.removeEntityCallback(id, watch);
	}

	watchApp(path, callback) {
		let point = this._appWatches, node;
		if (!callback) { throw Error('ERR_WATCH_NO_CALLBACK'); }

		for (node of path) {
			if (!point[node]) { point[node] = {}; }
			point = point[node];
		}


		if (!point._watches) { point._watches = []; }
		point._watches.push(callback);
		callback(this.getApp(path));
	}
	...
	_processAppWatches(changes, value, watches) {
		if (!value) { value = this.app; }
		if (!watches) { watches = this._appWatches; }
		if (watches._watches) {
			for (const fn of watches._watches) {
				fn(value);
			}
		}
		...
		for (const prop in changes) {
			if (watches[prop]) {
				this._processAppWatches(changes[prop], value[prop], watches[prop]);
			}
		}
	}
};
