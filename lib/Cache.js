...
class Cache {
	...
	_checkFilter(entity, prop, value) {
	...
	getKeyValues(entity) {
		...
		for (const type in this._types) {
			...
			for (const key in this._types[type].base) {
				...
				for (const prop in slice.filter) {
					if (
						typeof entity[prop] !== 'undefined' && !this._checkFilter(entity, prop, slice.filter[prop])
					) {
						continue keyloop; //eslint-disable-line
					}
				}
				...
				if (slice.link) { for (const linkType in slice.link) {
					const dKey = this.getDualKey(slice, linkType);
			...
			for (const key in this._types[type].link) {
				const slice = this._types[type].link[key],
					dKey = this.getDualKey(slice, type),
					prim = this.indexes[dKey] && this.indexes[dKey][entity.id],
					value = prim && prim[slice.order];
			...
			for (const key in this._types[type].join) {
				const slice = this._types[type].join[key],
					prim = this.entities[entity[slice.join[type]]],
					value = prim && prim[slice.order];
	...
	*/
	putEntity(id, entity, excludeKeys) {
		...
		if (this.entities[id]) {
			...
			entity = jsonop(this.entities[id], entity, this._options.entityOp); // eslint-disable-line
		}
...
//						this.indexes[key].put({ [type]: null });
	...
	putIndex(key, knowledge, items) {
		...
		if (slice.link) { for (const type in slice.link) {
		...
		if (slice.join) { for (const type in slice.join) {
		...
		if (slice.join || slice.link) {
			for (const item of items) {
				for (const name in item) { if (item[name]) {
	...
	getDualKey(slice, type) {
		if (slice.join && slice.join[type]) {
			const prop = slice.join[type];
	...
	_doesEntityBelongToKey(entity, slice) {
		for (const prop in slice.filter) {
			if (!this._checkFilter(entity, prop, slice.filter[prop])) {
				return false;
			}
		}
		return true;
	...
	indexEntities(key) {
		...
		for (const id in this.entities) {
			...
			if (
				...
				!slice.join && !slice.link && this._doesEntityBelongToKey(entity, slice)
			) {
	...
	*/
	put(changes) {
		...
		if (!this._options.changes) {
			allChanges = new Cache({ changes: true });
		...
		if (changes.knowledge) {
			...
			for (const key in changes.knowledge) {
				...
				if (!this._options.changes) allChanges.put(ch);
			}
		}
		...
		if (this._options.changes) {
			if (changes.auth) this.auth = changes.auth;
			return;
		}
			...
			for (const key in allChanges.knowledge) {
				if (this._queryCallbacks[key]) {
					...
					this._queryCallbacks[key].forEach(qcb => { // eslint-disable-line no-loop-func
						...
						const items = this.query(key, range);
		...
		if (this._changes) {
			this._changes.put(allChanges);
		} else {
			this._changes = allChanges;
	...
	_notifyChanges() {
		let changes = {};
		if (Object.keys(this._changes.knowledge).length) {
			changes.knowledge = this._changes.knowledge;
		}
		if (Object.keys(this._changes.indexes).length) {
			changes.indexes = this._changes.indexes;
		}
		if (Object.keys(this._changes.entities).length) {
			changes.entities = this._changes.entities;
		}
		if (Object.keys(this._changes.state).length) {
			changes.state = this._changes.state._data;
		}
		if (this._changes.auth) {
			changes.auth = this._changes.auth;
		}
		if (Object.keys(this._changes.queries).length > 1 || Object.keys(this._changes.queries.entities).length) {
			changes.queries = this._changes.queries;
		}
	...
	getItemsGaps(key, slice, range) {
		...
		if (slice.link || slice.join) {
			items = new OrderedArray(
				...
				items.arr.map(item => {
					const cItem = { [slice.type]: item };

					if (slice.link) { for (const type in slice.link) {
						cItem[type] = this.getEntity(item[slice.link[type]]);
						if (cItem[type].type === 'loading') {
							gaps.missing = true;
						}
					} }

					if (slice.join) { for (const type in slice.join) {
						cItem[type] = this.getDiscrete(
							this.getDualKey(slice, type),
							item.id
						);
						if (cItem[type].type === 'loading') {
							gaps.missing = true;
