	...
	*/
	...
	countedToBounded(range3, items) {
		...
		if (selectedItems.length >= range3.before + range3.after) {
			if (!Number.isFinite(range3.start)) range2.arr[0].end = range3.start;
			else if (!Number.isFinite(range3.end)) range2.arr[0].start = range3.end;
			return range2;
		}
		if (range3.before === 0) {
	...
	*/
	...
	getItemsGaps(key, slice, range) {
		...
		if (!this.knowledge[key]) {
			...
			gaps = (
				range.length === 3 && items.length ? items.getRange() : ranges
