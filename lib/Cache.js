...
class Cache {
	...
	_checkFilter(entities, prop, value) {
		const parts = prop.split('_');
		if (parts.length === 1) {
			if (entities.every(entity => !(prop in entity))) { return true; }
			return entities.some(entity => entity[prop] === value);
		} else {
			const field = parts.slice(0, parts.length - 1).join('_');

			if (entities.every(entity => !(field in entity))) { return true; }
			// This hack is used to prevent missing entities of a compound item
			// from failing a filter meant for other entities.

			return entities.some(entity => {
				switch (parts[parts.length - 1]) {
				case 'cts':
					if (!(entity[field] instanceof Array)) return false;
					return value.every(val => entity[field].indexOf(val) > -1);
				case 'pref':
					return entity[field].startsWith(value);
				case 'olp':
					if (!(entity[field] instanceof Array)) return false;
					return value.some(val => entity[field].indexOf(val) > -1);
				default:
					return true;
				}
			});
	...
	getKeyValues(entity) {
		...
		for (const type in this._types) {
			...
			for (const key in this._types[type].link) {
				...
				for (const prop in slice.filter) {
					if (!this._checkFilter(
						[ entity, prim ], prop, slice.filter[prop]
					)) continue keyloop; //eslint-disable-line
				}
			...
			for (const key in this._types[type].join) {
				...
				if (this.knowledge[key].contains(value)) {
					keyValues[key] = { value, refreshed: true };
					keyValues[this.getDualKey(slice, type)] = {
		...
		if (!this.knowledge[key]) {
			...
			if (slice.link) {
				for (const type in slice.link) {
					...
					addType('linkd', type, this.getDualKey(slice, type));
				}
			}
			if (slice.join) {
				for (const type in slice.join) {
					addType('join', type, key, slice);
					addType('joind', type, this.getDualKey(slice, type));
				}
			}
