...
//						this.indexes[key].put({ [type]: null });
	...
	*/
	...
	getEntity(id, callback) {
		...
		if (result) {
			...
			if (callback) {
				...
				(this._queryCallback.entities = this._queryCallback.entities || []).push({ callback, watch: false});
			}
	...
	watchEntity(id, callback) {
		...
		this._queryCallback.entities = this._queryCallback.entities || {};
		(this._queryCallback.entities = this._queryCallback.entities || []).push(watch);
	...
	watchApp(path, callback) {
		...
		callback(null, this.getApp(path));
	}
...
/*
    ...
    for (const prop in changes) {
        if (watches[prop]) {
            return processAppWatches(changes[prop], value[prop], watches[prop]);
        }
    }
}
