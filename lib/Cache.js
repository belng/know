	...
	*/
	putEntity(entity, excludeKeys) {
		...
		if (this.entities[id]) {
			...
			entity = jsonop(this.entities[id], entity, this.options.entityOp);
		}
	...
	putIndex(key, knowledge, items) {
		...
		if (this._queryCallbacks[key]) {
			for (let i in this._queryCallbacks[key]) {
				...
				if (!watch && ranges.subtract(this.knowledge[key]).length === 0) {
	...
	*/
	put(changes) {
		...
		function mergeChanges(ch) {
			...
			allChanges.knowledge.merge(ch.knowledge);
		}
		
		mergeChanges(allChanges, changes);
		
		/* handle the addition of ranges of results */
		if (changes.knowledge) { for (let key in changes.knowledge) {
			let items = changes.indexes && changes.indexes[key] || [];
		...
		if (changes.app) {
			this.app = jsonop(this.app, changes.app, this.options.appOp);
		}

		/* handle the addition of a query */
		if (changes.queries) { for (let key in changes.queries) {
			if (!this.queries[key]) {
		...
		if (allChanges.knowledge) { for (const key in allChanges.knowledge) {
			if (this._queryCallbacks[key]) {
				const { range, callback, watch } = this._queryCallbacks[key];
				if (watch) {
					// fire callback
				}
			}
		...
		if (allChanges.entities) { for (const id in allChanges.entities) {
			if (this._queryCallbacks.entities[id]) {
				// TODO: fire, delete if not watching.
			}
		}}
	...
	getItemsGaps(key, range, callback, watch) {
	...
	watch (key, range, callback) {
		...
		let { items, gaps } = getItemsGaps (key, range, callback, true);
		setImmediate(() => callback(null, items));
		
		this._queryCallbacks[key] = this._queryCallbacks[key] || [];
		let index = this._queryCallbacks[key].push({ range, callback, watch: true }) - 1;
