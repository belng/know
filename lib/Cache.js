...
class Cache {
	constructor(options) {
		...
		this._unWatchListeners = [];

		this.keyToSlice = keyToSlice;
		this.sliceToKey = sliceToKey;
		this.arrayOrder = arrayOrder;
		this.countedToBounded = RangeArray.countedToBounded;
	...
	*/
	fillItem(slice, item) {
		...
		for (const source in this._changes) {
			entities = jsonop.merge(
				entities,
				this._changes[source].entities
			);
		}
		...
		if (!(slice.type in item)) {
			for (const type in item) {
				...
				if (slice.link && type in slice.link) {
					...
					if (item[slice.type] === null) delete item[slice.type];
					break;
				} else if (slice.join && type in slice.join) {
					const id = entity[slice.join[type]];
					item[slice.type] = id in entities ?
						entities[id] : this.getEntity(id);
					if (item[slice.type] === null) delete item[slice.type];
					break;
				} else {
		...
		if (slice.type in item) {
			...
			if (slice.link) {
				for (const lType in slice.link) {
					...
					const id = entity[slice.link[lType]];
					item[lType] = id in entities ?
						entities[id] : this.getEntity(id);
					if (item[lType] === null) delete item[lType];
				}
			}
			...
			if (slice.join) {
				for (const jType in slice.join) {
					...
					if (item[jType] === null) delete item[jType];
				}
			}
			return item;
		} else {
...
//						this.indexes[key].put({ [type]: null });
	...
	putIndex(key, knowledge, items) {
		...
		if (slice.join) {
			for (const type in slice.join) {
				...
				items.arr.forEach(item => {
					if (item[type]) {
						index[item[slice.type].id] = item[type];
					} else {
						index[item[slice.type].id] = null;
					}
					// if (item && item[type] && item[type][prop]) {
	...
	*/
	put(changes) {
		...
		/* Aggregate changes and fire onChange() when done. */

		const source = changes.source || '__default__';
		const allChanges = new Change();

		if (!this._changes[source]) {
			this._changes[source] = new Change();
			Promise.resolve().then(this.notifyChanges);
		}

		allChanges.put(changes);
		this._changes[source].put(changes);
		...
		if (changes.knowledge) {
			...
			for (const key in changes.knowledge) {
				...
				if (!items) {
					items = new OrderedArray(arrayOrder(slice), []);
				} else if (!(items instanceof OrderedArray)) {
					changes.indexes[key] = items =
						new OrderedArray(arrayOrder(slice), items);
				}
			...
			for (const key in allChanges.knowledge) {
				if (this._queryCallbacks[key]) {
					...
					this._queryCallbacks[key].forEach(qcb => { // eslint-disable-line no-loop-func
						...
						range = RangeArray.countedToBounded(
							range, this.indexes[key], this.knowledge[key]
						);
	...
	_notifyWatch(options) {
		this._watchListeners.forEach(fn => fn(options));
	}

	_notifyUnWatch(options) {
		this._unWatchListeners.forEach(fn => fn(options));
	}
			...
			items = new OrderedArray(
				...
				items.arr.map(item => {
					...
					for (const type in cItem) {
						if (cItem[type] && cItem[type].type === 'loading') {
							cItem[type] = null;
							// gaps.missing = true;
						}
					}
	...
	query (slice, range, callback) {
		...
		if (gaps.length) {
			let diff = [];
			if (this.queries[key]) {
				diff = this.queries[key].difference(gaps);
			} else {
				diff = gaps;
			}
	...
	watch (slice, range, callback) {
		...
		return () => {
			...
			this._notifyUnWatch({ slice, range });
		};
	}
	...
	onChange(fn) {
		...
		return () => {
			const i = this._changeListeners.indexOf(fn);
			if (i > -1) {
	...
	onWatch(fn) {
		this._watchListeners.push(fn);
		return () => {
			const i = this._watchListeners.indexOf(fn);
			if (i > -1) {
				this._watchListeners.splice(i, 1);
			}
		};
	}

	onUnWatch(fn) {
		this._unWatchListeners.push(fn);
		return () => {
			const i = this._unWatchListeners.indexOf(fn);
			if (i > -1) {
				this._unWatchListeners.splice(i, 1);
			}
		};
	}
	...
	getDiscrete(key, val, callback) {
		...
		if (entity || entity === null) {
			...
			if (callback) { setImmediate(() => callback(null, entity)); }
			// return null; // Until discrete queries are supported.
...
Cache.arrayOrder = arrayOrder;
Cache.keyToSlice = keyToSlice;
