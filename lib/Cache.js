...
class Cache {
	...
	_checkFilter(entity, prop, value) {
		...
		if (parts.length === 1) {
			...
			if (!(field in entity)) { return true; }
			// This hack is used to prevent one entity of a compound item
			// from failing a filter meant for another entity.
	...
	*/
	getKeyValues(entity) {
		...
		console.log("put entity fired: ",entity);
		if (!entity) return {};
		...
		for (const type in this._types) {
			...
			for (const key in this._types[type].base) {
				...
				console.log("gKV 1: ", key, slice, value, entity);
				if (typeof value === 'undefined') { continue; }

				for (const prop in slice.filter) {
					console.log("gKV 2: ", prop, !this._checkFilter(entity, prop, slice.filter[prop]));

					if (
						!this._checkFilter(entity, prop, slice.filter[prop])
					) {
				...
				console.log("GKV-3: ", keyValues[key]);
				if (slice.link) {
	...
	*/
	putEntity(id, entity) {
		...
		if (this.entities[id]) {
			...
			entity = jsonop.apply(this.entities[id], entity, this._options.entityOp); // eslint-disable-line
		}
		console.log("entity: ", entity, this.entities[id]);
		...
		if (oldKeyValues) {
			for (const key in oldKeyValues) {
				if (
					!newKeyValues[key] ||
					oldKeyValues[key].value !== newKeyValues[key].value
				) {
					...
					this.indexes[key].delete(newKeyValues[key].value);
	...
	*/
	...
	_notifyChanges() {
		...
		for (const source in this._changes) {
			changes.put(this._changes[source]);
		}
