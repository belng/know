...
//						this.indexes[key].put({ [type]: null });
	...
	*/
	...
	query (slice, range, callback) {
		let key = this.sliceToKey(slice), dRange = new RangeArray([ range ]), r = dRange.get(0),
			{ items, gaps } = this.getItemsGaps(key, slice, r);
	...
	watch (slice, range, callback) {
		...
		let key = this.sliceToKey(slice), { items } = this.getItemsGaps(key, slice, range, callback, true),
