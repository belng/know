	...
	*/
	put(changes) {
		...
		if (changes.entities) { for (let id in changes.entities) {
			if (changes.entities[id] === null) this.deleteEntity(changes.entities[id]);
	...
	query(key, range, callback) {
		...
		if (!this.knowledge[key]) {
			...
			if (range.length === 3 && items.length < range[1] + range[2]) {
				...
				console.log("position: ",pos, range);
				if (
					pos < range[1] &&
					!this.knowledge[key].hasStart()
				) { console.log("prepend", items); gaps.add([
					items.get(0)[items.order], range[1] - pos, 0
				]); }
				console.log("Params:", items, pos, range[2]);
				if (
					items.length - pos < range[2] &&
					!this.knowledge[key].hasEnd()
				) { console.log("append"); gaps.add([
