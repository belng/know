...
class Cache {
	constructor(options) {
		...
		if (options) { for (const i in defaults) {
	...
	_checkFilter(entity, prop, value) {
		...
		if (parts.length === 1) {
			...
			case 'cts':
				...
				break;
			case 'pref':
				return !!entity[field].startsWith(value.replace(/%*$/, ''));
			case 'olp':
				for (let i = 0; i < value.length; i++) {
					if ((entity[field].indexOf(value[i]) >= 0)) {
						return true;
	...
	*/
	put(changes) {
		...
		if (Object.keys(this.entities).length > this._options.maxEntities) {
			console.info('MAXENTITIES Clearing');
			this.clear();
		...
		let source = allChanges.source ? allChanges.source : '__default__';
	...
	_notifyChanges() {
		let changes = jsonop({}, this._changes);
