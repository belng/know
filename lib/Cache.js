...
//						this.indexes[key].put({ [type]: null });
	...
	putIndex(key, knowledge, items) {
		...
		if (!this.knowledge[key]) {
			...
			changes = jsonop.merge(changes, this.indexEntities(key));
		} else {
		...
		if (slice.join || slice.link) {
			for (const item of items) {
				for (const name in item) {
					if (item[name]) {
						...
						changes = jsonop.merge(changes, this.putEntity(id, item[name], [ key ]));
					}
				}
			}
		} else {
			for (const item of items) {
				...
				changes = jsonop.merge(changes, this.putEntity(id, item, [ key ]));
			}
		}
	...
	*/
	...
	_notifyChanges() {
		const changes = jsonop.merge({}, this._changes);
