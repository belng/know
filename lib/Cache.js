...
//						this.indexes[key].put({ [type]: null });
	...
	*/
	...
	countedToBounded(range3, items) {
		if (range3.length === 2) return new RangeArray(range3);

		console.log("arguments to slice", ...range3);
		...
		if (range3.before === 0) {
			...
			range2.start = -Infinity;
		} else {
			let pivot = selectedItems.indexOf(range3.start);
			range2.start = pivot < range3.before ? -Infinity : selectedItems.valAt(0);
			range2.end = selectedItems.length - pivot < range3.end ? +Infinity :
