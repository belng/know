...
//						this.indexes[key].put({ [type]: null });
	...
	*/
	put(changes) {
		...
		if (changes.updates) {
			for (const id in changes.updates) {
				if (changes.entities && changes.entities[id]) continue;
				const entity = this.getEntity(id),
					update = changes.updates[id];
				if (entity === null) {
					throw Error("Update on nonexistent entity " + id);
				}
				if (entity.type === 'loading') continue;
				// It will load the updated version anyway.
				changes.entities = changes.entities || {};
				changes.entities[id] = jsonop.apply(entity, update);
			}
		}
