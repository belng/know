	...
	indexEntities(key) {
		...
		for (let id in this.entities) {
			let entity = this.entities[id],
				value = entity[slice.order];
			if (typeof value !== 'undefined' && this._is(entity, slice.type)) {
	...
	*/
			...
			for (const key in allChanges.knowledge) {
				if (this._queryCallbacks[key]) {
					...
					this._queryCallbacks[key].forEach(qcb => { // eslint-disable-line no-loop-func
						...
						const items = this.indexes[key].slice(...range);

						if (
							items.getRange().intersect(allChanges.knowledge[key])
							.length
						) {
							if (watch) {
								callback(items);
	...
	query (slice, range, callback) {
		...
		if (gaps.length) {
			...
			if (callback) {
				setImmediate(() => callback(null, items.arr));
			}
		}
	...
	watch (slice, range, callback) {
		...
		setImmediate(() => callback(items.arr));
